...
/**
 * Thin wrapper around `navigator.geolocation.getCurrentPosition`. Returns
 * a state machine the UI can react to: idle → loading → ready / denied /
 * unavailable / error.
 *
 * We deliberately don't watch position — for a hailing UX the user picks
 * a pickup once and then commits. Continuously moving the marker on
 * top-of-the-form would be more noise than signal.
 */
