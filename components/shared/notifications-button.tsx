...
interface NotificationsButtonProps {
  ...
  /** Visual treatment for the button. `glass` is used inside dark headers. */
  tone?: Tone;
 ...
 * ESC, or route navigation (handled at the link level via `onClick={close}`
 * inside the list items).
 *
 * The button intentionally has no `disabled` state — even with an empty
 * feed the sheet opens and explains what'll show up here. That's far better
 * UX than a dead button.
 */
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className={cn(
          "relative grid place-items-center transition-colors active:scale-95 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary-container focus-visible:outline-offset-2",
          tone === "glass"
            ? "glass-panel p-sm rounded-lg hover:bg-white/10"
            : "h-11 w-11 rounded-full text-on-surface-variant hover:bg-white/5 hover:text-on-surface",
          className,
        )}
      >
        <MaterialIcon
