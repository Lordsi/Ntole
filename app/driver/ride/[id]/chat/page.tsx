...
export default async function DriverChatPage({
  ...
  if (!ride) notFound();
  if (ride.driver_id !== profile!.id && profile!.role !== "admin") redirect("/driver");

  return (
    <div className="flex h-screen flex-col bg-background">
      <header className="flex items-center gap-3 border-b border-white/5 p-3">
        <Link href={`/driver/ride/${id}`} aria-label="Back">
          <IconButton size={36}>
            <ArrowLeftIcon className="h-4 w-4" />
          </IconButton>
        </Link>
        <span className="text-sm font-semibold">Chat with rider</span>
      </header>
